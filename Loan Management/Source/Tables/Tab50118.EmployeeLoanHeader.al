table 50118 "Employee Loan Header"
{
  ...
  //  DrillDownPageID = "Employee Loan List";
    ...
    fields
    {
        ...
        field(11; "Approval Status"; Enum "Loan Approval Status")
        {
    ...
    begin
        if "Requested Amount" > "Maximum Amount" then
            Error('Requested amount %1 exceeds maximum amount %2', 
                "Requested Amount", "Maximum Amount");
    end;
    
    local procedure ValidateLoanTerm()
    begin
        if "Term Months" > "Maximum Term Months" then
            Error('Requested term %1 months exceeds maximum term %2 months', 
                "Term Months", "Maximum Term Months");
    end;
